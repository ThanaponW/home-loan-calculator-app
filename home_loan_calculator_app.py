...
# --- Simple Visitor Counter (ย้ายมาไว้ส่วนท้ายของหน้า) ---
# กำหนดเส้นทางไปยังไฟล์นับจำนวนผู้เข้าชม
VISITS_FILE = "visits.txt"

# อ่านจำนวนผู้เข้าชมปัจจุบัน
current_visits = 0
if os.path.exists(VISITS_FILE):
    try:
        ...
        st.error(f"เกิดข้อผิดพลาดในการอ่านจำนวนผู้เข้าชม: {e}")

# เพิ่มจำนวนผู้เข้าชมเฉพาะในเซสชันนี้
# ตรวจสอบว่ายังไม่ได้นับในเซสชันปัจจุบัน เพื่อหลีกเลี่ยงการนับซ้ำเมื่อหน้ามีการโหลดใหม่
if "counted_visit" not in st.session_state:
    current_visits += 1
    st.session_state["counted_visit"] = True # ทำเครื่องหมายว่าได้นับแล้วสำหรับเซสชันนี้
    try:
        with open(VISITS_FILE, "w") as f:
            f.write(str(current_visits))
    except Exception as e:
        st.error(f"เกิดข้อผิดพลาดในการบันทึกจำนวนผู้เข้าชม: {e}")

# แสดงผลจำนวนผู้เข้าชมที่ส่วนท้ายของหน้า
st.markdown("---") # เพิ่มเส้นคั่นเพื่อความเรียบร้อย
st.markdown(f"**จำนวนผู้เข้าชมแอปทั้งหมด:** {current_visits} ครั้ง")
# --- สิ้นสุดส่วน Simple Visitor Counter ---